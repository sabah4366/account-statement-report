...
class CollectionSaleReportPdf(models.AbstractModel):
        ...
        if rec_data:
            for rec in rec_data:
                pdc_ids = rec.pdc_payment_ids.filtered(
                    lambda x: x.state in ['registered', 'deposited'] and x.payment_date >= datetime.strptime(data['start_date'], "%Y-%m-%d").date() and
    ...
    def _get_statement_details(self, data):
        ...
        for rec in rec_data:
            ...
            pdc_amt = 0
            for pdc in pdc_ids:
                pdc_amt += pdc.payment_amount
            if not pdc_ids:
                print("not pdc",rec)
                if rec.amount_residual_signed != 0:
                    print("recccccccjjj :",rec)
    ...
    def _get_entry_total(self, data):
        ...
        customer_data = self.env['account.move'].search(
            [('company_id', '=', self.env.company.id), ('partner_id', '=', data['partner_id']),
             ('move_type', '=', 'out_invoice'),('invoice_date', '<=', data['end_date']),('invoice_date', '>=', data['start_date']),('state', '=', 'posted'),
             ('payment_state', 'in', ['not_paid', 'partial'])],
            order="invoice_date asc")
    ...
    def _get_total(self, data):
        ...
        total=pdc_tot
        print(total, "total1")
        grand_total =  abs(grand_tot) - abs(total)
